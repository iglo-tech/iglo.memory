...
describe('command contract', () => {
  test('only the five commands and explicit reset are accepted', () => {
    ...
    expect(parseArguments(['init', '--reset-credentials'])).toEqual({
      name: 'init',
      resetCredentials: true,
    });
    for (const name of ['prepare', 'status', 'gc'] as const)
      expect(parseArguments([name])).toEqual({ name });
    expect(parseArguments(['search', '  auth tokens  '])).toEqual({
      name: 'search',
      query: '  auth tokens  ',
    });
  });
  test('invalid arguments are rejected without reflecting them', () => {
    for (const args of [
      [],
      ['help'],
      ['init', '--api-key', 'DUMMY_SECRET'],
      ['prepare', '--reset-credentials'],
      ['search'],
      ['search', '  '],
      ['search', '--api-key'],
      ['search', 'a', 'b'],
  ...
  test('unexpected failures never expose the exception message', () => {
    expect(errorResponse(new Error('DUMMY_SECRET'))).toEqual({
