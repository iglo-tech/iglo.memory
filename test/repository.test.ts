...
function fixture() {
  const path = mkdtempSync('/tmp/iglo-t01-');
  fixtures.push(path);
  return path;
}
...
test('relative linked gitfile uses worktree root, not common administration', () => {
  ...
  mkdirSync(linked);
  const metadata = join(root, '.git', 'worktrees', 'linked');
...
test('bare, absent, malformed and symlink markers fail', () => {
  ...
  const bare = join(enclosing, 'bare');
  admin(bare);
  expect(() => resolveWorktree(bare)).toThrow();
  expect(() => resolveWorktree(fixture())).toThrow();
  const root = repo();
  writeFileSync(join(root, '.git', 'HEAD'), 'invalid');
  expect(() => resolveWorktree(root)).toThrow();
  const linked = fixture();
  symlinkSync(join(root, '.git'), join(linked, '.git'));
  expect(() => resolveWorktree(linked)).toThrow();
});
...
test('invalid configuration values fail', () => {
  for (const value of [
    ...
    {},
    { project: 'p' },
    { project: ' ', embedding: { model: 'm' } },
    { project: 'p\ninjected', embedding: { model: 'm' } },
    { project: 'p', embedding: { model: '' } },
    { project: 'p', embedding: { model: 'm\0' } },
  ]) {
...
test('missing, malformed and static symlink configuration paths fail', () => {
  ...
  expect(() => readConfig(root)).toThrow();
  rmSync(path);
  const target = join(fixture(), 'config');
  writeFileSync(target, '{"project":"p","embedding":{"model":"m"}}');
  symlinkSync(target, path);
  expect(() => readConfig(root)).toThrow();
  rmSync(join(root, '.agent'), { recursive: true });
  symlinkSync(fixture(), join(root, '.agent'));
  expect(() => readConfig(root)).toThrow();
});

test('ordinary source folders with administrative-looking names remain inside the worktree', () => {
  const root = repo();
  const source = join(root, 'fixtures');
  admin(source);
