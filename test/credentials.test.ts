...
test('shared saved credentials, override precedence and explicit replacement', () => {
  const home = fixture();
  saveCredential(' first-dummy ', home);
  ...
  expect(resolveCredential(home, ' override-dummy ')).toEqual({
    source: 'environment',
    key: 'override-dummy',
  });
  expect(readFileSync(path, 'utf8')).toContain('first-dummy');
  saveCredential('second-dummy', home);
  expect(resolveCredential(home, '')?.key).toBe('second-dummy');
...
test('reject repository-contained storage and symlinks; reset may replace malformed safe contents', () => {
  ...
  expect(() => saveCredential('dummy', home)).toThrow();
  const safe = fixture();
  mkdirSync(join(safe, '.config'));
  symlinkSync(fixture(), join(safe, '.config/iglo.mem'));
  expect(() => saveCredential('dummy', safe)).toThrow();
  const other = fixture();
  saveCredential('dummy', other);
  const path = join(other, '.config/iglo.mem/credentials.json');
  writeFileSync(path, '{');
  expect(() => resolveCredential(other, '')).toThrow();
  saveCredential('replacement', other);
  expect(resolveCredential(other, '')?.key).toBe('replacement');
});
...
test('real init JSON, saved reuse across repositories, preservation and noninteractive failure', async () => {
  ...
  const missing = await cli(first, home, ['init']);
  expect(missing.exit).toBe(1);
  expect(missing.value.error.code).toBe('API_KEY_MISSING');
  const path = join(first, '.agent/memory.json');
  const config = readFileSync(path, 'utf8');
  const env = await cli(first, home, ['init'], 'dummy-override');
  expect(env.exit).toBe(0);
  expect(env.value.credentialSource).toBe('environment');
  expect(env.value.credentialsSaved).toBe(false);
  expect(env.stdout + env.stderr).not.toContain('dummy-override');
  expect(readFileSync(path, 'utf8')).toBe(config);
  saveCredential('shared-dummy', home);
  for (const root of [first, second]) {
    const result = await cli(root, home, ['init']);
    expect(result.exit).toBe(0);
    expect(result.value.credentialSource).toBe('saved');
  ...
  expect(reset.exit).toBe(1);
  expect(reset.value.error.code).toBe('SETUP_REQUIRES_TTY');
  ...
  const isolated = await cli(first, home, ['init']);
  expect(isolated.exit).toBe(0);
  expect(isolated.value.credentialSource).toBe('saved');
...
test('concurrent successful saves leave one complete value and last completed save wins', async () => {
  ...
  saveCredential('final-dummy', home);
  expect(resolveCredential(home, '')?.key).toBe('final-dummy');
});
...
test('ordinary dotfiles repositories below home reject saved credential reads and writes', async () => {
  for (const relative of ['.config', '.config/iglo.mem']) {
    for (const gitfile of [false, true]) {
      const home = fixture();
      saveCredential('existing-dummy', home);
      const repo = join(home, relative);
      ...
      if (gitfile) writeFileSync(marker, 'gitdir: /fixture/administration\n');
        ...
        mkdirSync(join(marker, 'refs'));
        writeFileSync(join(marker, 'HEAD'), 'ref: refs/heads/main\n');
      }
      const file = join(home, '.config/iglo.mem/credentials.json');
      ...
      expect(result.value.error.code).toBe('CREDENTIALS_INVALID');
      expect(readFileSync(file)).toEqual(old);
      expect(resolveCredential(home, 'override-dummy')?.source).toBe('environment');
    }
  }
