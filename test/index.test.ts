...
test('real offline empty prepare/search/status/gc CLI and no implicit refresh', async () => {
  ...
  const prepared = await cli(root, home, ['prepare']);
  expect(prepared.exit).toBe(0);
  expect(prepared.value.chunks).toBe(0);
...
test('prepare/reuse/edit/failure/delete/GC with snapshot-only search and orphan reuse', async () => {
  const root = setup();
  const path = join(root, '.agent/knowledge/auth.md');
  writeFileSync(path, '# Tokens\nRotate the refresh token.\n\n# Cookies\nStore cookies securely.');
  const calls: string[][] = [];
  const embedding = async (inputs: string[]) => {
    calls.push(inputs);
    return inputs.map((input) => (input.includes('Cookies') ? [0, 1] : [1, 0]));
  };
  const first = await prepare(root, config, embedding, () => 'dummy');
  expect(first.embeddedVectors).toBe(2);
  expect(calls).toHaveLength(1);
  const old = readFileSync(join(indexPath(root), 'snapshot.json'));
  const second = await prepare(root, config, embedding, noCredential);
  expect(second.reusedVectors).toBe(2);
  expect(calls).toHaveLength(1);
  const before = readFileSync(path);
  writeFileSync(path, before.toString().replace('Rotate', 'Renew'));
  const changed = await prepare(root, config, embedding, () => 'dummy');
  expect(changed.embeddedVectors).toBe(1);
  expect(changed.reusedVectors).toBe(1);
  ...
  const result = await search(
    ...
    'refresh token',
    async (inputs) => {
      expect(inputs).toEqual(['refresh token']);
      return [[1, 0]];
    },
    () => 'dummy',
    {
  ...
  writeFileSync(path, before.toString().replace('Rotate', 'Renew'));
  rmSync(join(indexPath(root), 'snapshot.json'));
  expect((await prepare(root, config, embedding, noCredential)).reusedVectors).toBe(2);
  rmSync(path);
  expect((await prepare(root, config, embedding, noCredential)).chunks).toBe(0);
...
test('missing vector status, corrupt vector failure, profile mismatch and safe GC authority', async () => {
  const root = setup();
  writeFileSync(join(root, '.agent/knowledge/a.md'), 'alpha');
  const embedding = async () => [[1, 0]];
  await prepare(root, config, embedding, () => 'dummy');
  const snapshot = readSnapshot(root, config);
  const vector = join(indexPath(root), 'vectors', snapshot.chunks[0]!.vector);
  rmSync(vector);
  expect((await status(root, config)).missingVectors).toBe(1);
  await expect(search(root, config, 'a', embedding, noCredential)).rejects.toThrow(
    'index is invalid',
  );
  await expect(gc(root, config)).rejects.toThrow('index is invalid');
  await prepare(root, config, embedding, () => 'dummy');
  writeFileSync(vector, Buffer.alloc(8));
  await expect(status(root, config)).rejects.toThrow('index is invalid');
  expect(() => readSnapshot(root, { ...config, embedding: { model: 'changed' } })).toThrow(
    'incompatible',
  );
  const files = readdirSync(join(indexPath(root), 'vectors'));
  writeFileSync(join(indexPath(root), 'snapshot.json'), '{');
  await expect(gc(root, config)).rejects.toThrow('index is invalid');
  expect(readdirSync(join(indexPath(root), 'vectors'))).toEqual(files);
  for (const field of [
    { source: '../../escape.md' },
    { startLine: 0 },
    { text: 'tampered' },
    ...
    expect(() =>
      parseSnapshot({ ...snapshot, chunks: [{ ...snapshot.chunks[0], ...field }] }, config),
    ).toThrow();
  }
});
...
test('65 unique inputs use 64+1 batches and a failed later batch leaves reusable orphans', async () => {
  const root = setup();
  for (let i = 0; i < 65; i++)
    writeFileSync(join(root, '.agent/knowledge', `${i}.md`), 'entry ' + i);
  const calls: number[] = [];
  await expect(
    prepare(
      root,
      ...
      async (inputs) => {
        ...
        if (calls.length === 2) throw new Error('FAIL');
        return inputs.map(() => [1, 0]);
      },
      () => 'dummy',
    ),
  ).rejects.toThrow('FAIL');
  expect(calls).toEqual([64, 1]);
  expect(() => readSnapshot(root, config)).toThrow('No prepared index');
  ...
  expect(result.reusedVectors).toBe(64);
  expect(result.embeddedVectors).toBe(1);
});
...
test('simultaneous real command processes publish and read complete worktree-local snapshots', async () => {
  ...
  const second = setup();
  const home = fixture();
  await cli(first, home, ['prepare']);
  await cli(second, home, ['prepare']);
  const before = readFileSync(join(indexPath(second), 'snapshot.json'));
  for (let round = 0; round < 3; round++) {
    const results = await Promise.all([
      cli(first, home, ['prepare']),
      cli(first, home, ['search', 'query']),
      cli(first, home, ['status']),
      cli(first, home, ['gc']),
    ]);
...
test('binary loader retains float32 values and rejects zero/nonfinite data even with matching digest', async () => {
  const { readVector, vectorBytes } = await import('@/src/store');
  const { sha256 } = await import('@/src/chunks');
  const root = setup();
  writeFileSync(join(root, '.agent/knowledge/a.md'), 'alpha');
  await prepare(
    root,
    config,
    async () => [[Math.fround(0.1), Math.fround(0.2)]],
    () => 'dummy',
  );
  const snapshot = readSnapshot(root, config);
  const chunk = snapshot.chunks[0]!;
  const receipt = { ...chunk, profile: snapshot.profile };
  const vector = readVector(root, receipt);
  expect(Array.from(vector)).toEqual([Math.fround(0.1), Math.fround(0.2)]);
  ...
  for (const values of [
    ...
    expect(() => readVector(root, { ...receipt, vectorHash: 'sha256:' + sha256(bytes) })).toThrow(
      'index is invalid',
    );
  }
});
...
test('committed populated snapshots work in fresh linked worktrees with concurrent source-unreadable searches', async () => {
  ...
  try {
    for (const child of children) {
      ...
      expect(result.results[0].source).toBe('.agent/knowledge/passage.md');
      expect(Array.from(result.results[0].snippet).length).toBe(401);
      expect(result.results[0].snippet).not.toBe(text);
    }
    for (const [i, path] of [root, linked].entries())
      expect(readFileSync(join(indexPath(path), 'snapshot.json'))).toEqual(originals[i]!);
  } finally {
    for (const child of children) {
