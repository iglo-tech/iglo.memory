...
export function directory(path: string, create = false, mode = 0o755): void {
  if (create) { try { mkdirSync(path, { mode }); } catch (error) { if (!(error instanceof Error && 'code' in error && error.code === 'EEXIST')) throw error; } }
...
export function checkAncestors(path: string): void {
  const absolute = resolve(path); let part = parse(absolute).root;
...
export function readBytes(path: string): Buffer {
  ...
  try { if (!fstatSync(fd).isFile()) throw new Error('Invalid file'); return readFileSync(fd); }
  finally { closeSync(fd); }
}
export function atomicWrite(path: string, bytes: string | Uint8Array, mode = 0o644): void {
  const temp = join(dirname(path), `.iglo-${randomUUID()}.tmp`);
  let created = false;
  try {
    ...
    created = false;
  } finally { if (created) { try { unlinkSync(temp); } catch { /* Preserve original failure. */ } } }
}
