...
import { atomicWrite, checkAncestors, directory, exists, readBytes, record } from '@/src/files';
...
function secure(path: string, kind: 'file' | 'directory') {
  ...
  if (
    (kind === 'file' ? !s.isFile() || s.nlink > 1 : !s.isDirectory()) ||
    (s.mode & 0o077) !== 0 ||
...
function location(home: string, create: boolean): string {
  try {
    ...
    const config = join(physical, '.config');
    const app = join(config, 'iglo.mem');
    checkAncestors(app);
...
export function resolveCredential(
  ...
  try {
    ...
    if (
      !record(value) ||
      !record(value.openrouter) ||
      typeof value.openrouter.apiKey !== 'string' ||
      !value.openrouter.apiKey.trim()
    )
      throw new Error();
...
export function saveCredential(key: string, home = homedir()): void {
  ...
  try {
    atomicWrite(path, JSON.stringify({ openrouter: { apiKey: key.trim() } }) + '\n', 0o600);
  } catch {
    throw new AppError('CREDENTIALS_SAVE_FAILED');
  }
}
...
async function hiddenEntry(): Promise<string> {
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const finish = (key?: string) => {
      ...
      if (key === undefined) reject(new AppError('SETUP_CANCELLED'));
      else resolve(key);
    };
    const cancel = () => finish();
...
export async function setupCredentials(
  reset: boolean,
): Promise<{ credentialSource: Credential['source']; credentialsSaved: boolean }> {
  if (reset && (!process.stdin.isTTY || !process.stderr.isTTY))
    throw new AppError('SETUP_REQUIRES_TTY');
  if (!reset) {
    const current = resolveCredential();
    if (current) {
      if (current.source === 'environment')
        process.stderr.write(
          'Using OPENROUTER_API_KEY; it has not been saved for other processes.\n',
        );
  ...
  if (reset)
    process.stderr.write(
      'Credentials replaced. OPENROUTER_API_KEY still takes precedence when set.\n',
    );
