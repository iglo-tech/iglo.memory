const messages = {
  ARGUMENT_INVALID:
    'Use iglo.mem init [--reset-credentials], prepare, search "<query>", status, or gc.',
  REPOSITORY_INVALID: 'Run iglo.mem inside a valid Git worktree.',
  CONFIG_INVALID:
    'Invalid or missing .agent/memory.json. Run iglo.mem init or repair the configuration.',
  API_KEY_MISSING:
    'No API key is available. Run iglo.mem init interactively or supply OPENROUTER_API_KEY.',
  CREDENTIALS_INVALID:
    'Saved credentials or their location are invalid. Repair ~/.config/iglo.mem/credentials.json and its owner-only permissions, or use OPENROUTER_API_KEY.',
  CREDENTIALS_SAVE_FAILED:
    'Could not save credentials. Check the user credentials directory and try init again.',
  SETUP_REQUIRES_TTY:
    'Credential reset requires an interactive terminal. Run iglo.mem init --reset-credentials interactively.',
